...
internal class MineDiagnosticsLog(private val startedAtMillis: Long = System.currentTimeMillis()) {
    ...
    fun record(
        ...
        rows += listOf(
            ...
            f(tick?.materialMs),
            f(tick?.discoveryMs),
            tick?.classification?.candidateSamples ?: 0,
            tick?.classification?.newMaterialSamples ?: 0,
            tick?.classification?.nearbyExistingSegments ?: 0,
            tick?.classification?.tunnelBoundsChecks ?: 0,
            tick?.classification?.existingTunnelChecks ?: 0,
            tick?.classification?.nearbyOreBodies ?: 0,
            tick?.classification?.oreFieldEvaluations ?: 0,
            f(tick?.segmentLengthMetres),
            f(tick?.newExcavatedVolumeCubicMetres),
            f(tick?.oreVolumeCubicMetres),
            f(tick?.wasteRockVolumeCubicMetres),
        ).joinToString(",")
    }
    ...
    fun toCsv(): String = buildString {
        appendLine(
            "elapsed_ms,event,digging,depth_m,removed_m3,ore_m3,fps,frame_ms," +
                ...
                "rock_chunks,ore_chunks,rock_queue,ore_queue,rock_busy,ore_busy,tick_ms," +
                "material_ms,discovery_ms,candidate_samples,new_samples,nearby_tunnel_segments," +
                "tunnel_bounds_checks,exact_tunnel_checks,nearby_ore_bodies,ore_field_evals," +
